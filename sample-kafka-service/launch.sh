...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "sample-kafka-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name sample-kafka-service apps/sample-kafka-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app sample-kafka-service:1.0-SNAPSHOT
    oc expose svc/sample-kafka-service
else

	echo "Launching the application locally..."
	pattern="sample-kafka-service"
	files=( $pattern )
	cd ${files[0]}
