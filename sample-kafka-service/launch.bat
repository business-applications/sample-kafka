...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "sample-kafka-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name sample-kafka-service apps/sample-kafka-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app sample-kafka-service:1.0-SNAPSHOT
		call oc expose svc/sample-kafka-service
	) else (
